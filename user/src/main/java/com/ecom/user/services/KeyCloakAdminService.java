...
public class KeyCloakAdminService {
    ...
    public String createUser(String token, UserRequest userRequest){
        ...
        if(location == null) throw new RuntimeException("Keycloak did not return Location header to find user id: " + response.getBody());
    ...
    public void assignRealmRoleToUser(String username, String roleName, String userId){
        ...
        if(!response.getStatusCode().is2xxSuccessful()){
            throw new RuntimeException("Failed to assign realm role " + roleName +" to user: " + username + " with HTTP code: " + response.getStatusCode());
        }
